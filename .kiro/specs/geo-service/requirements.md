# Requirements Document — Geo Service (Role 3)

## Introduction

The Geo Service is the geospatial and integrations layer of EIFS. It converts raw location strings into lat/lon coordinates via Nominatim, provides proximity-based candidate incident lookup for the dedup pipeline, supplies map-ready cluster data to the frontend, and owns the ERSS mock script and sample dataset.

## Glossary

- **Geo_Service**: The FastAPI service (port 8002) that geocodes location strings and performs spatial queries.
...
- **Proximity_Window**: Spatial and temporal bounds for candidate lookup — 500 m radius, 30-minute time window.
- **Cluster**: A group of incidents within close proximity displayed as a single map marker at low zoom levels.
- **ERSS_Mock**: A standalone Python script that emits synthetic report events to the Backend ingest endpoint.
- **Incident_Store**: The SQLite (local) or PostgreSQL (production) database holding incident records.
...
2. THE Geo_Service SHALL compute distances using the Haversine formula.
...
2. WHEN `lat` and `lon` are provided, THE endpoint SHALL first fetch nearby candidates using the proximity window, then pass them to the AI Service `/dedup` endpoint to get semantic similarity scores.
...
5. THE `area_name` field SHALL be populated by reverse-geocoding the cluster centroid using Nominatim.

---

### Requirement R3-5: Area Labelling (Reverse Geocoding)
...
1. THE Geo_Service team SHALL provide a JSON file containing at least 20 geocoded sample incidents with fields: `incident_id`, `incident_type`, `lat`, `lon`, `severity_label`, `area_name`, `report_count`.
2. THE sample dataset SHALL include incidents spread across at least 5 distinct areas of one Indian city (Chennai or Bengaluru).
3. THE sample dataset SHALL be committed to the repository at `data/sample_incidents.json`.
